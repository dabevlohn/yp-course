...
fn fix(
    ...
    bytes_to_fix
        ...
        .map(|orig_word| {
            if percent_idx
                < (get_bytes_diff(bytes_to_fix, orig_word) * 100)
                    / bytes_to_fix.len()
            {
            ...
            let first_char = if shall_start_from_capital {
            ...
            if let Some(label) = set_of_labels.iter().find(|label| {
                to_lower_iter(&label).eq(to_lower_iter(orig_word))
            }) {
                EitherIt::It1(label.chars().chain([' ']))
            } else {
                EitherIt::It2(
                    first_char
                        .chain(to_lower_iter(remaining.as_str()).chain([' '])),
                )
            }
        })
        .flatten()
    ...
    if parts_count == 1 {
        ...
        if idx == 0 {
            ...
            [
                ...
                split_n_impl(bytes, idx + 1, to_idx, (parts_count + 1) / 2),
            ]
            .concat()
        }
    }
